...
cache::result cache::check_cache_for_node(const std::string& node) {
   ...
   auto hashed_item = calculate_hash(node, _probe_nonce);
   ...
   return probe_registrar(hashed_item, node);
}
...
cache::result cache::check_registrar(const size_t hash, 
   ...
   for (auto& s : sensors) {
      if (s.id == sensor) {
         ...
         // We found the match, so plop it in the caches
         cache_item(hash);
         return result::KNOWN;
      }
   }
...
cache::result cache::probe_registrar(const size_t hash, 
                                       const std::string& node)
{
   httplib::Client cli(_registrar_address, _registrar_port);
   
   auto result = cli.Get("/probe/" + node);
   ...
   try {
      json::jobject::tryparse(result->body.c_str(), json_result);
   ...
   if (json_result.has_key("status")) {
      ...
      if (json_result.has_key("data")) {
         ...
         if (json_result["data"].as_string() == "not found") {
            return result::UNKNOWN_NODE;
         } else if (json_result["data"].as_string() == "found") {
...
void cache::cache_item(const size_t hash) {
