...
helper_c::result helper_c::submit(node_v1_c& node) {
   ...
   std::string submission = "/registrar/add/" + id + "/" + encoded_node;
   return issue_command(submission);
}

helper_c::result helper_c::submit(controller_v1_c& controller) {

   std::string encoded_controller;
   if (!controller.encode_to(encoded_controller)) {
      return result::INVALID_DATA;
   }
...
helper_c::result helper_c::remove(const std::string& id) {
   ...
   std::string submission = "/registrar/delete/" + id;
   return issue_command(submission);
}

helper_c::result helper_c::retrieve(const std::string& id, node_v1_c& node) {
   
   auto result = _http_client->Get("/registrar/fetch/" + id);
...
helper_c::result helper_c::retrieve(const std::string& id, controller_v1_c& controller) {
   ...
   controller.clear();
   if (!controller.decode_from(json_result)) {
      return result::FAILURE;
   }
